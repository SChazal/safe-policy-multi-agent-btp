...
class SafetyLayer:
    ...
    def _initialize_constraint_models(self):
        self._models = [ConstraintModel(self._env.observation_space["agent_position"].shape[0]+self._env.observation_space["rival_position"].shape[0],
    ...
    def _sample_steps(self, num_steps):
        ...
        for step in range(num_steps):            
            action_1 = self._env.action_space.sample()
            action_2 = self._env.action_space.sample()
            ...
            c_1_next, c_2_next = self._env.get_constraint_values()

            self._replay_buffer.add({
                "action": action_1,
                "observation": np.concatenate([observation_1["agent_position"], observation_1["rival_position"]] ),
                "c": c_1,
                "c_next": c_1_next 
            })
            ...
            self._replay_buffer.add({
                "action": action_2,
                "observation": np.concatenate([observation_2["agent_position"], observation_2["rival_position"]] ),
                "c": c_2,
                "c_next": c_2_next 
            })
    ...
    def _update_batch(self, batch):
        ...
        # batch = self._replay_buffer.sample(self._config.batch_size) <--- redundant?
    ...
    def get_safe_action(self, observation, action, c):    
        ...
        self._eval_mode()
        g = [x(self._as_tensor(np.concatenate([observation["agent_position"], observation["rival_position"]])).view(1, -1)) for x in self._models]
        self._train_mode()

        if self._config.use_gpu:
            g = [x.detach().cpu() for x in g]
